...
class Test_d_2_r(unittest.TestCase):
    ...
    def test_nominal(self) -> None:
        self.assertEqual(space.d_2_r(180.), pi)

    @unittest.skipIf(not HAVE_NUMPY, 'Skipping due to missing numpy dependency.')
    def test_vector(self) -> None:
        np.testing.assert_array_almost_equal(space.d_2_r(np.array([0., 45., 90., 180.])), np.array([0., pi/4, pi/2, pi]), 14)
...
class Test_r_2_d(unittest.TestCase):
    ...
    """
    def test_nominal(self) -> None:
        self.assertEqual(space.r_2_d(pi), 180.)

    @unittest.skipIf(not HAVE_NUMPY, 'Skipping due to missing numpy dependency.')
    def test_vector(self) -> None:
        np.testing.assert_array_almost_equal(space.r_2_d(np.array([0., pi/4, pi/2, pi])), np.array([0., 45., 90., 180.]), 14)
...
class Test_norm(unittest.TestCase):
    ...
    """
    def test_nominal(self) -> None:
        mag = space.norm(np.array([3., 4.]))
        self.assertEqual(mag, 5.)

    def test_vectors(self) -> None:
        mag = space.norm(np.array([[3., 0.], [4., 10.]]))
        np.testing.assert_array_equal(mag, np.array([5., 10.]))
...
class Test_jd_to_numpy(unittest.TestCase):
    ...
    def test_vectorized(self) -> None:
        ...
        jd = space.numpy_to_jd(x)  # type: ignore[arg-type]
...
class Test_aerospace_d_2_dms(unittest.TestCase):
    ...
    def test_nominal(self) -> None:
        ...
        np.testing.assert_array_almost_equal(dms, np.array([38., 27., 15.]), 11)

    def test_vector(self) -> None:
        dms = space.d_2_dms(np.array([0., 38.4541666666666666]))
        np.testing.assert_array_almost_equal(dms, np.array([[0., 0., 0], [38., 27., 15.]]).T, 11)
...
class Test_aerospace_dms_2_d(unittest.TestCase):
    ...
    """
    def test_nominal(self) -> None:
        dms = space.dms_2_d(np.array([38., 27., 15.]))
        np.testing.assert_array_almost_equal(dms, 38.4541666666666666, 11)

    def test_vector(self) -> None:
        dms = space.dms_2_d(np.array([[0., 0., 0], [38., 27., 15.]]).T)
        np.testing.assert_array_almost_equal(dms, np.array([0., 38.4541666666666666]), 11)
...
class Test_aerospace_hms_2_r(unittest.TestCase):
    ...
    @unittest.skipIf(not HAVE_NUMPY, 'Skipping due to missing numpy dependency.')
    def test_vector(self) -> None:
        x = space.hms_2_r(np.array([[0., 0., 0], self.hms, [8., 0., 0.], [23, 59, 60]]).T)
        np.testing.assert_array_almost_equal(x, np.array([0., self.exp, 2*pi/3, 2*pi]), 11)
...
class Test_aerospace_r_2_hms(unittest.TestCase):
    ...
    @unittest.skipIf(not HAVE_NUMPY, 'Skipping due to missing numpy dependency.')
    def test_vector(self) -> None:
        r = np.array([0., self.r, 2*pi/3, 2*pi])
        hms = space.r_2_hms(r)
        exp = np.array([[0., 0., 0], self.exp, [8., 0., 0.], [24, 0, 0]]).T
