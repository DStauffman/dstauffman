...
class Test_plot_health_time_history(unittest.TestCase):
    ...
    def test_nominal(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, label=self.label, \
            units=self.units, opts=self.opts, legend=self.legend))

    def test_defaults(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label))

    def test_with_units(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label, self.units))

    def test_with_opts(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label, opts=self.opts))

    def test_with_legend(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label, legend=self.legend))

    def test_no_data(self):
        with dcs.capture_output() as out:
            dcs.plot_health_time_history(self.time, None, '')
        output = out.getvalue().strip()
        out.close()
        self.assertEqual(output, 'plot skipped due to missing data.')

    def test_ignore_zeros(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label, ignore_empties=True))
    ...
    def test_ignore_zeros3(self):
        ...
        with dcs.capture_output() as out:
            not_a_fig = dcs.plot_health_time_history(self.time, self.data, label='All Zeros', ignore_empties=True)
        output = out.getvalue().strip()
        out.close()
    ...
    def test_bad_legend(self):
        with self.assertRaises(AssertionError):
            dcs.plot_health_time_history(self.time, self.data, self.label, legend=self.legend[:-1])

    def test_second_yscale1(self):
        self.figs.append(dcs.plot_health_time_history(self.time, self.data, self.label, units='population', \
    ...
    def test_single_point(self):
        self.figs.append(dcs.plot_health_time_history(self.time[1:], self.data[1:,:], self.label))
    ...
    def test_not_ndarray(self):
        self.figs.append(dcs.plot_health_time_history(0, 0, 'Zero'))

    def test_0d(self):
        self.figs.append(dcs.plot_health_time_history(np.array(0), np.array(0), 'Zero'))

    def test_1d(self):
        self.figs.append(dcs.plot_health_time_history(np.arange(5), np.arange(5), 'Line'))
    ...
    def test_3d(self):
        ...
        self.figs.append(dcs.plot_health_time_history(self.time, data3, self.label, opts=self.opts))

    def test_bad_4d(self):
        bad_data = np.random.rand(self.time.shape[0], 4, 5, 1)
        with self.assertRaises(AssertionError):
            dcs.plot_health_time_history(self.time, bad_data, self.label, opts=self.opts)
...
class Test_plot_health_monte_carlo(unittest.TestCase):
    ...
    def test_normal(self):
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data, self.label, self.units)
    ...
    def test_bad_truth_size(self):
        ...
        with self.assertRaises(ValueError):
            dcs.plot_health_monte_carlo(self.time, self.data, self.label, self.units, \
                truth=self.truth)
        # close uncompleted plot window
        plt.close(plt.gcf())

    def test_opts(self):
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data, self.label, self.units, opts=self.opts)

    def test_diffs(self):
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data_matrix, self.label, self.units, \
            plot_as_diffs=True)
    ...
    def test_colormap(self):
        self.opts.colormap = 'Dark2'
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data, self.label, self.units, self.opts)
    ...
    def test_array_data1(self):
        ...
        self.fig = dcs.plot_health_monte_carlo(self.time, data, self.label, self.units)

    def test_array_data2(self):
        data = np.column_stack((self.data, self.data))
        self.fig = dcs.plot_health_monte_carlo(self.time, data, self.label, self.units, plot_as_diffs=True)
    ...
    def test_second_yscale2(self):
        second_yscale = {'New ylabel [units]': 100}
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data, self.label, units='percentage', \
            second_yscale=second_yscale)

    def test_simple(self):
        self.fig = dcs.plot_health_monte_carlo(0, 0, 'Text')

    def test_plot_empty(self):
        self.fig = dcs.plot_health_monte_carlo([], [], '')

    def test_plot_all_nans(self):
        self.fig = dcs.plot_health_monte_carlo(np.array([np.nan, np.nan]), np.array([np.nan, np.nan]), self.label)
    ...
    def test_skip_plot_sigmas(self):
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data, self.label, self.units, plot_sigmas=0)

    def test_plot_confidence(self):
        self.fig = dcs.plot_health_monte_carlo(self.time, self.data_matrix, self.label, self.units, plot_confidence=0.95)

    def test_not_ndarray(self):
        self.fig = dcs.plot_health_monte_carlo(0, 0, 'Zero')

    def test_0d(self):
        self.fig = dcs.plot_health_monte_carlo(np.array(0), np.array(0), 'Zero')

    def test_1d(self):
        self.fig = dcs.plot_health_monte_carlo(np.arange(5), np.arange(5), 'Line')

    def test_bad_3d(self):
        bad_data = np.random.rand(self.time.shape[0], 4, 5)
        with self.assertRaises(ValueError):
            dcs.plot_health_monte_carlo(self.time, bad_data, self.label, opts=self.opts)
