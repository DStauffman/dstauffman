...
class Test_aerospace_two_line_elements(unittest.TestCase):
    ...
    def test_file(self) -> None:
        ...
        for i in range(len(lines) // 3):  # TODO: vectorize this function so this loop can be eliminated
            ...
            self.assertGreater(elements.e, 0.0)  # type: ignore[misc]
            self.assertLess(elements.e, 1.0)  # type: ignore[misc]
...
class Test_aerospace_rv_2_oe(unittest.TestCase):
    ...
    def test_nominal(self) -> None:
        ...
        for key in self.floats:
            self.assertIsInstance(getattr(elements1, key)[0], np.float64, f"Bad key: {key}")
        for key in self.enums:
            self.assertEqual(getattr(elements1, key)[0], space.OrbitType.elliptic, f"Bad key: {key}")
        for key in self.bools:
            self.assertIsInstance(getattr(elements1, key)[0], np.bool_, f"Bad key: {key}")
        for key in self.dates:
            self.assertIsInstance(getattr(elements1, key)[0], np.datetime64, f"Bad key: {key}")
