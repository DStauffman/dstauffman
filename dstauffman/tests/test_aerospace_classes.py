...
class Test_aerospace_KfInnov(unittest.TestCase):
    ...
    def test_combine_inplace(self) -> None:
        innov1 = space.KfInnov(name='Gnd', units='m', chan=['X', 'Y', 'Z'], num_innovs=60, num_axes=3, time_dtype=NP_DATETIME_FORM)
        innov1.fploc = np.random.rand(2, 60)
        innov1.snr = np.random.rand(60)
        innov2 = space.KfInnov(name='Gnd', units='m', chan=['X', 'Y', 'Z'], num_innovs=50, num_axes=3, time_dtype=NP_DATETIME_FORM)
...
class Test_aerospace_Kf(unittest.TestCase):
    ...
    def test_combine_nominal(self) -> None:
        kf1 = space.Kf(name='Name 1', units='m', chan=('a', 'b', 'c', 'd', 'e', 'f'), innov_chan=('X', 'Y'), \
            num_points=30, num_states=6, time_dtype=float, num_innovs=10, num_axes=2)
        kf1.time[:] = np.arange(30.)  # type: ignore[index]
        kf1.innov.time[:] = np.arange(10., 20.) + 0.2
        kf1.innov.innov[:] = np.random.rand(2, 10)
        kf2 = space.Kf(name='Name 2', units='m', chan=('a', 'b', 'c', 'd', 'e', 'f'), innov_chan=('X', 'Y'), \
            num_points=60, num_states=6, time_dtype=float, num_innovs=30, num_axes=2)
        kf2.time[:] = np.arange(30., 90.)  # type: ignore[index]
        kf2.innov.time[:] = np.arange(40., 70.) + 0.2
        kf2.innov.innov[:] = np.random.rand(2, 30)
        kf3 = kf1.combine(kf2)
        self.assertEqual(kf3.name, 'Name 1')
        self.assertEqual(kf3.chan, ('a', 'b', 'c', 'd', 'e', 'f'))
        np.testing.assert_array_equal(kf3.time, np.arange(90.))
    ...
    def test_combine_to_empty(self) -> None:
        ...
        kf1.innov.units = 'm'
        kf1.innov.chan = ('X', 'Y')
        kf2 = space.Kf(name='Name 2', units='m', chan=('dx', 'dy', 'dz'), innov_chan=('X', 'Y'), \
            num_points=60, num_states=3, num_innovs=60, num_axes=2, time_dtype=float)
        kf3 = kf1.combine(kf2)
    ...
    def test_combine_inplace(self) -> None:
        kf1 = space.Kf(name='Name 1', units='rad', chan=('a', 'b', 'c', 'd', 'e', 'f'), innov_chan=('X', 'Y', 'Z'), \
            num_points=30, num_states=6, num_innovs=10, num_axes=3, time_dtype=NP_DATETIME_FORM)
        ...
        kf2 = space.Kf(name='Name 2', units='rad', chan=('a', 'b', 'c', 'd', 'e', 'f'), innov_chan=('X', 'Y', 'Z'), \
            num_points=60, num_states=6, num_innovs=30, num_axes=3, time_dtype=NP_DATETIME_FORM)
    ...
    def test_chop(self) -> None:
        kf = space.Kf(name='Gnd', chan=('dx', 'dy', 'dz'), innov_chan=('X', 'Y'), units='m', \
            num_points=60, num_states=3, num_innovs=110, num_axes=2, time_dtype=float)
        kf.time[:] = np.arange(60.)  # type: ignore[index]
        kf.innov.time[:] = np.arange(5., 60., 0.5)
        ...
        self.assertEqual(kf3.innov.units, 'm')
        self.assertEqual(kf3.innov.chan, ('X', 'Y'))
        np.testing.assert_array_equal(kf2.time, np.arange(10., 20.))
        np.testing.assert_array_equal(kf2.innov.time, np.arange(10., 20., 0.5))
        np.testing.assert_array_equal(kf3.time, np.arange(20., 41.))
        np.testing.assert_array_equal(kf3.innov.time, np.arange(20., 40.5, 0.5))
    ...
    def test_chop_inplace(self) -> None:
        ...
        np.testing.assert_array_equal(kf2.time, np.arange(10., 21.))
        self.assertEqual(kf2.att.shape, (4, 11))  # type: ignore[union-attr]
        np.testing.assert_array_equal(kf2.innov.time, np.arange(10., 20.5, 0.5))
    ...
    def test_chop_return_ends(self) -> None:
        ...
        np.testing.assert_array_equal(kf2.innov.time, np.arange(10., 20.5, 0.5))
        self.assertEqual(kf2.innov.innov.shape, (2, 21))
        np.testing.assert_array_equal(kf3.time, np.arange(0., 10.))
        self.assertEqual(kf3.att.shape, (4, 10))  # type: ignore[union-attr]
        np.testing.assert_array_equal(kf3.innov.time, np.arange(5., 10., 0.5))
        self.assertEqual(kf3.innov.innov.shape, (2, 10))
        np.testing.assert_array_equal(kf4.time, np.arange(21., 60.))
        self.assertEqual(kf4.att.shape, (4, 39))  # type: ignore[union-attr]
        np.testing.assert_array_equal(kf4.innov.time, np.arange(20.5, 60., 0.5))
...
class Test_aerospace_KfRecord(unittest.TestCase):
    ...
    def test_combine_nominal(self) -> None:
        ...
        kf_record1.time[:] = np.arange(30.)  # type: ignore[index]
        kf_record2 = space.KfRecord(num_points=60, num_states=9, num_active=6, num_axes=3, time_dtype=float)
        kf_record2.time[:] = np.arange(30., 90.)  # type: ignore[index]
        kf_record3 = kf_record1.combine(kf_record2)
        np.testing.assert_array_equal(kf_record3.time, np.arange(90.))
    ...
    def test_combine_to_empty(self) -> None:
        ...
        kf_record2.time[:] = np.arange(30., 90.)  # type: ignore[index]
        kf_record3 = kf_record1.combine(kf_record2)
        np.testing.assert_array_equal(kf_record3.time, np.arange(30., 90.))
    ...
    def test_chop(self) -> None:
        ...
        kf_record.time[:] = np.arange(60.)  # type: ignore[index, union-attr]
